...
RUN apt-get update -y && apt-get install -y build-essential gcc make git curl wget \
...
RUN SECRET_KEY_BASE="$(mix phx.gen.secret)" DATABASE_URL="ecto://postgres:postgres@localhost:5432/pdf_dev" mix app.start
...
RUN apt-get update -y && \
  apt-get install -y libstdc++6 openssl libncurses5 locales ca-certificates tesseract-ocr wget poppler-utils \
  && apt-get clean && rm -f /var/lib/apt/lists/*_*
